...
class ShotBoundaryDetection:
    ...
    def __detect_multi_step_comparison_scheme(self,
        ...
        def calc_phi_eta(alg_atr_l):
            ...
            # noinspection PyShadowingNames
            mu = np.array(mu, dtype=type(mu))
            shape = mu.shape
            mu.resize([shape[0],shape[1]])
            mu = np.matrix(mu)
    ...
    def detect(self, set_progress=None, finish=None):
        ...
        while cut_index < len(cuts) and gradual_index < len(gradual_transitions):
            if cuts[cut_index] < gradual_transitions[gradual_index][0]:
                self.sb.append({'transition': 'hard cut',
                                'cut_frame': cuts[cut_index]+16})
                cut_index += 1
            else:
                self.sb.append({'transition': 'dissolve',
                                'start_frame': gradual_transitions[gradual_index][0]+12,
                                'end_frame': gradual_transitions[gradual_index][1]+12})
                gradual_index += 1
